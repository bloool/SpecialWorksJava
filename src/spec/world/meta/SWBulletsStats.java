        ...
        return table -> {
            ...
            for(T t : orderedKeys){
                ...
                table.table(bt -> {
                    ...
                    //do the sentries first because they need different types of stats
                    if(type instanceof Sentry b){
                        sep(bt, Core.bundle.format("sentry.spec-j-sentry-lifetime", b.lifetime));
                        sep(bt, Core.bundle.format("sentry.spec-j-sentry-reload", b.reloadTime));
                        sep(bt, Core.bundle.format("sentry.spec-j-sentry-shots", b.shots));
                        sep(bt, Core.bundle.format("sentry.spec-j-sentry-inaccuracy", b.inaccuracy));
                        sep(bt, Core.bundle.format("sentry.spec-j-sentry-shoots"));

                        bt.row();

                        ammo(ObjectMap.of(t, b.shootType), indent + 1).display(bt);
                    }else{
